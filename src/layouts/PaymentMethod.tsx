...
//   onClose: () => void;
//   total: number;
//   onConfirm: (paymentMethodId: string) => void; // Nova prop
// }
...
//     if (selectedPayment) {
//       onConfirm(selectedPayment); // Chama o fluxo da modal de confirmação
//     } else {
...
// Mock do ButtonPrimary
interface PaymentMethodProps {
  isOpen: boolean; // Add this line
  onClose: () => void;
  total: number;
  onConfirm: (paymentMethodId: string, paymentMethodName: string) => void;
}

const paymentMethods = [
  { id: "pix", name: "Pix", icon: SmartphoneIcon, color: "bg-teal-600" },
  {
    id: "credit",
    name: "Cartão de Crédito",
    icon: CreditCard,
  ...
  { id: "cash", name: "Dinheiro", icon: DollarSignIcon, color: "bg-green-600" },
];
...
const PaymentMethod = ({
  ...
  const handlePaymentConfirm = () => {
    ...
    if (selectedPayment && selectedMethod) {
      onConfirm(selectedPayment, selectedMethod.name);
    } else {
      alert("Selecione uma forma de pagamento válido");
    }
  };
